...
###  vector/list/deque之间的琐碎事

1. **vector**: 结构,特性,弱点.

   - **构造**: std::vector在底层维护了**3个指针和一块连续的内存**: 分别是**当前元素位置指针,内存块地址指针,内存尾后指针(也可以当作容量指针).**当前元素指针到达尾后指针时候,按1.5倍或者2倍进行扩容,MSVC和GCC的扩容机制略有不同. 发生扩容时候,所有元素会重新发生拷贝,因此有必要预留足够的空间.
   - **特性**:  内存连续  ---> **高效的CPU访问效率. 可以用索引实现随机访问, 复杂度O(1)**.
   - **指针稳定性**: 插入操作可能导致内存重新分配, 删除操作可能导致迭代器失效.
   - **弱点**: 插入效率(**尤其是内存块前,中部分**,不是尾),会导致元素的大量移动. 内存使用效率不行,因为不会自动适应容量,内存块可能浪费.

2. **list**: 结构,特性,弱点

   - **构造**: 内部维护了一个双向链表,list是一个完整的容器.元素由每个节点保存.
   - **特性**: 每个元素有3个内容,两个指针,一个value,**内存不连续,相邻节点之间通过前后指针联系**,而不是直接的物理内存相邻,**节点的内存单独分配**
   - **优点**: 任意位置高效插入(**但是先要找到前后指针,或者满足某条件**),  迭代器稳定-->不会发生删除节点后,迭代器失效,也没有扩容的失效问题,  不需要预分配.
   - **缺点**: 内存开销大(每个节点需要额外保留两个指针的大小空间), 内存碎片化(CPU的访问效率大大降低), 不能随机访问,只能从头遍历.

   > 根据list的特点, 适用于高效插入/删除操作的情景.或者大对象的存储,因为没有内存重新分配的难点.差不多刚好和vector相反.

3. **deque**: 

   - **构造**: 若干数据块(存储数据的实际内存),一个中央控制器(记录所有内存块的首地址)
   - **特性**: 内存在**逻辑上连续**,物理上部分连续(**块内连续,块间不连续**),内存分配机制--> 直接分配一个内存块. 较为高效的访问(低于vector),首尾增加删除元素高效(优于vector,但是中间插入数据和vector都不好)
   - **有很多vector的优点,但是也不能完全上位替代**: 结构比vector更复杂, 机制比vector也复杂,和vector很相似,只需要注意一些特殊的区别就好.

   > 但是vector用的比deque多很多.所以**正常使用直接就用vector就行了, 除非十分契合deque的特性**,否则直接使用vector简单且高效.
...
- g++	-o	out	main.cpp	-L . 	-ldemo	 -lanother		#lanother是其他的库链接选项

**编译动态库及其使用:**

- g++    -fPIC    -c    demo.cpp    -o    demo.o
- g++    -shared    -o    libdemo.so    demo.o

动态库的使用方法:

- g++    -o    out    main.cpp    -L .    -ldemo

> 注意, 编译选项不同于静态库,静态库使用的是ar 制作库文件,但是动态库始终都是g++/gcc制作. 先使用fPIC, 然后使用shared命令. 不过两者都是使用L查找路径, l链接具体的库文件, L . 的意思是链接路径就在当前目录. (当然, **默认安装在/usr/local/lib或者/usr/lib目录下**不需要-L路径, **设置了环境变量之后也不需要**), 最直接也最准确的方法还是直接编译指令写全.



**关于这两种库在代码中时机使用的本质区别:**

- 静态库: **直接将代码嵌入到可执行文件中**, 编译结束之后可以直接删除所有的.o和.a文件
- 动态库不能删除.so文件.动态库**只是创建了符号链接**, 代码还是会到动态库中寻找实际函数的定义.

> 动态库更方便维护, 可以直接更新动态库的文件, 而不必重新编译整个项目, 只需要保证ABI(应用二进制接口)保持兼容即可. 并且文件大小也是用动态库编译出来的文件更小.
